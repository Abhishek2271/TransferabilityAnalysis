...
class Model(ModelDesc):
    ...
    def build_graph(self, image, label):
        ...
        # get the logits layer. Logits are final layer values which are passed to softmax function. 
        ...
        #create a operation "output" that computes tensor "output:0" that is softmax output of the logits
        tf.nn.softmax(logits, name="output")
        tf.add_to_collection("logits", logits)
        
        # a vector of length B with loss of each sample
        # Here to compute cross-entropy loss we use the tf.nn.sparse_softmax_cross_entropy_with_logits. This 
        #   is equivalent to using softmax and then sending it to find cross entropy loss. But tf.nn.softmax_cross_entropy_with_logits
        #   provides better mathematical stability than doing softmanx yourself and then using tf..nn.cross_entropy.
        #   further tf.nn.sparse_softmax_cross_entropy_with_logits, encodes our labels to one hot vectors so we dont have to do it manually
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=label)
        print("c r loss", loss)
        loss = tf.reduce_mean(loss, name='cross_entropy_loss')  # the average cross-entropy loss
        print("c r mean loss", loss)    
        #for now do not regularize the weight
        # Use a regex to find parameters to apply weight decay.
        # Here we apply a weight decay on all W (weight matrix) of all fc layers
        # more on regularize_cost on: https://tensorpack.readthedocs.io/en/latest/modules/models.html#tensorpack.models.regularize_cost
        ...
        #we can convert to float32 later when converting accuracy as well but the benefit of converting here is that 
            #we then do not need to again convert when computing train_error
