...
The difference here is that the channel number is dynamic
'''
...
class Model(ModelDesc):
    ...
    def build_graph(self, image, label):
        """
            ...
            for channel is required so add one additional channel at axis =3. The default tensorpack dataset function gets this dimension also
            but in __iter__ it again removes the dimension so in the end there is no channel dimension until this point in case of MNIST
        ...
        # here we are using a simple convolution net with 4 convolution layers and one dense layer as hidden layers
        # As per capacity MODEL A < MODEL B (100%) < MODEL C (200%)
        ...
        with remap_variables(quantize_weight), \
                argscope(Conv2D, kernel_size=3, activation=tf.nn.relu, filters=model_capacity):
                # LinearWrap is just a syntax sugar.
                # See tutorial at https://tensorpack.readthedocs.io/tutorial/symbolic.html
            logits = (LinearWrap(image)
                        .Conv2D('conv0').apply(activate)
                        .Conv2D('conv0.1').apply(activate)
                        .Conv2D('conv0.2')
                        .MaxPooling('pool0', 2).apply(activate)
                         #apply activation quantization for each activation  map, if a layer has max pooling then apply quantization after that
                        .Conv2D('conv1').apply(activate) 
                        .Conv2D('conv2').apply(activate)
                        .Conv2D('conv2.1').apply(activate)
                        .Conv2D('conv2.2')
                        .MaxPooling('pool1', 2).apply(activate) 
                        .Conv2D('conv3').apply(activate)
                        .Conv2D('conv3.1').apply(activate)
                        .Conv2D('conv3.2').apply(activate)
