...
This is a quantized version of the CIFAR model described in cifar_covnet.py (base cifar model)

CIFAR QUANTIZED MODEL
...
class Model(ModelDesc):
    ...
    def build_graph(self, image, label):       
        ...
        def quantize_weight(v):
            ...
            if not name.endswith('W') or 'conv1.1' in name or 'linear' in name:
                return v
            else:
        ...
        with remap_variables(quantize_weight), \
            ...
            logits = (LinearWrap(image) 
                        .Conv2D('conv1.1', filters=64, use_bias=True)
                        #.BatchNorm('bn0')
                        .apply(activate)
